...
def extract_json(s):
    ...
    # Try to parse JSON by finding the corresponding closing brace
    # We iterate through all closing braces to handle nested structures
    for i in range(len(s) - 1, start_pos, -1):
        if s[i] == "}":
            json_str = s[start_pos:i+1]
            try:
                parsed = json.loads(json_str)
            except json.JSONDecodeError:
                try:
                    # Fallback to literal_eval for single quotes or Python-specific syntax
